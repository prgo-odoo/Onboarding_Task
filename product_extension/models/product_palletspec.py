...
class ProductPalletSpecification(models.Model):
    ...
    cases_per_layer = fields.Float('Cases per Layer', required=True)
    layers_per_pallet = fields.Float('Layers per Pallet', required=True)
    qty = fields.Float('Pallet Quantity', compute='_compute_qty')
    product_uom_id = fields.Many2one('uom.uom', 'Unit of Measure', required=True)
    length = fields.Float('Pallet Length (m)', required=True)
    width = fields.Float('Pallet Width (m)', required=True)
    height = fields.Float('Pallet Height (m)', required=True)
    weight = fields.Float('Pallet Weight (kg)', required=True)
